...
def load_state_dict(model, ckpt_path, device="cuda", strict=False, assign=True):
    if Path(ckpt_path).suffix == ".safetensors":
        ckpt_path2 = os.path.join(folder_paths.models_dir, 'Step1x-Edit', ckpt_path) 
        state_dict = load_file(ckpt_path2, device)
    else:
...
class ImageGenerator:
    ...
    def input_process_image(self, img, img_size=512):
        ...
        return img_resized, pil_image.size
...
class Step1XEditNode:
    ...
    def Step1XEdit(self, image, prompt, seed, cfg, size_level, num_steps, step1x_edit_model, step1x_edit_model_vae, mllm_model):
        image_edit = ImageGenerator(
            ...
            qwen2vl_model_path=os.path.join(folder_paths.models_dir, 'MLLM', "Qwen2.5-VL-7B-Instruct"), 
            max_length=640,
        )
